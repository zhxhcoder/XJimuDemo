 ...
 * * @date 2018 年 1 月 19日
 * * <p>
 * * 收集 屏幕相关较全的 设备信息相关操作的工具类 @fork from QMUI_Android
 * Created by zhxh on 2018/11/2
 */
public class UIStatusBarHelper {
    ...
    public static boolean supportTransStatusBar() {
        return (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) && supportTransclentStatusBar6();
    }
    ...
    public static void setStatusBarHiddenWith(Window window, boolean hidden) {
...
//        return !(UIDeviceHelper.isZUKZ1() || UIDeviceHelper.isZTKC2016());
        return true;
    }
    ...
    private static void initStatusBarHeight(Context context) {
        ...
        if (UIDeviceHelper.isTablet(context)
                && sStatusbarHeight > UIDisplayHelper.dp2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP)) {
            //状态栏高度大于25dp的平板，状态栏通常在下方
            sStatusbarHeight = 0;
        } else {
            if (sStatusbarHeight <= 0
                    || sStatusbarHeight > UIDisplayHelper.dp2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP * 2)) {
     ...
     * 弹出的窗口是否覆盖状态栏
     */
    public static void fitPopupWindowOverStatusBar(boolean needFullScreen, PopupWindow popupWindow) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            try {
    ...
    public static void setStatusBarPaddingAndHeightInsertView(View insertView, Context context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (insertView != null) {
                insertView.getLayoutParams().height = UIStatusBarHelper.getStatusbarHeight(context);
            }
        }
    }

    /**
     * 重新上设置标题栏的高度使之延伸到页面 StatusBar 下
     *
     * @param toolBar
     */
    public static void setStatusBarPaddingAndHeight(View toolBar, Context context) {

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (toolBar != null) {
                int statusBarHeight = UIStatusBarHelper.getStatusbarHeight(context);
